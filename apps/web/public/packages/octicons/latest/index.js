    ...
    if (options) {
      ...
      if (options["width"] || options["height"]) {
        attrObj["width"] = options["width"]
          ? options["width"]
          : (parseInt(String(options["height"])) *
              Number(defaultOptions["width"])) /
            Number(defaultOptions["height"]);
        attrObj["height"] = options["height"]
          ? options["height"]
          : (parseInt(String(options["width"])) *
              Number(defaultOptions["height"])) /
            Number(defaultOptions["width"]);
      }
...
function closestNaturalHeight(naturalHeights, height) {
  ...
  return naturalHeights
    ...
    .reduce(
      (acc, naturalHeight) =>
        naturalHeight <= requestedHeight ? naturalHeight : acc,
      Number(naturalHeights[0]),
    );
}
